...
export default function cypher(hljs: HLJSApi): Language {
  ...
  // The negative lookahead keeps no-space keyword usage such as `MATCH(n)`
  // or `NOT(expr)` rendering as a keyword, not a function call.
  const FUNCTION_CALL: Mode = {
    begin: [
      new RegExp(
        `\\b(?!(?:${KEYWORDS.join("|")})\\()[A-Za-z_]\\w{0,60}(?:\\.\\w{1,60}){0,6}`,
      ),
      /\(/,
    ],
